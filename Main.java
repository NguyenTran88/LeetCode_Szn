...
class Main {
    ...
    public static List<Integer> toposort(List<Integer>[] graph) {
        ...
        for(int vertex = 1; vertex < graph.length;vertex++){
            ArrayList<Integer> graphRow = (ArrayList<Integer>)graph[vertex]; //we can cast ?
            for (Integer node : graphRow) {
                indegree[node++];
        ...
        //poll vs dequeue, enqueue vs add
        while(!q.isEmpty()){
            int vertex = q.poll();
            order.add(vertex);
            List<Integer> adjacentVetrtices = graph[vertex];
            for (int adjacent : adjacentVetrtices) {
        ...
        //look for cycle. 
        if(order.size() != graph.length-1){ //sandro has no task @ number 0
            return null;
        }
        return order;
    }
